IF EXISTS( SELECT 'Y' FROM sysobjects WHERE name = 'de_ui_extended_properties' AND TYPE = 'P')
BEGIN
	DROP PROC de_ui_extended_properties
END
GO
/************************************************************************************************/
...
/************************************************************************************************/
/* Modified by  :   Ponmalar A				Date: 20-Sep-2022			Defect ID : TECH-72927  */
/************************************************************************************************/
/* Modified by  :   Ponmalar A				Date: 27-Oct-2022			Defect ID : TECH-73996  */
/************************************************************************************************/
/* Modified by  :   Manoj S					Date: 01-Nov-2022			Defect ID : TECH-74072  */
/************************************************************************************************/
/* Modified by  :   Vimal Kumar R			Date: 02-Nov-2022			Defect ID : TECH-74150  */
/************************************************************************************************/
/* Modified by  :   Ponmalar A				Date: 01-Dec-2022			Defect ID : TECH-75230  */
/************************************************************************************************/
/* Modified by  :   Ponmalar A				Date: 03-Jan-2023			Defect ID : TECH-76231  */
/************************************************************************************************/
/* Modified by  :   Ponmalar A				Date: 09-Jan-2023			Defect ID : TECH-76402  */
/************************************************************************************************/
/* Modified by  :   Ponmalar A				Date: 02-Feb-2023			Defect ID : TECH-77097  */
/************************************************************************************************/
/* Modified by  :   Ponmalar A				Date: 13-Apr-2023			Defect ID : TECH-79742  */
/************************************************************************************************/
/* Modified by  :   Ponmalar A				Date: 03-May-2023			Defect ID : TECH-80135  */
/************************************************************************************************/
CREATE PROCEDURE de_ui_extended_properties
...
declare @placeholder		engg_description, 	@place_lang				engg_seqno,			@enabledefault		engg_name,			@hideSelect				engg_name
declare @hideinsert			engg_flag,			@hidedelete				engg_flag,			@hidecopy			engg_flag,			@hidecut				engg_flag,
		...
		@MobileGrid			engg_flag,			@Searchable				engg_flag,			@LiveClock			engg_flag,			@ClearTask				engg_flag,	--TECH-73996		
		@ShowAnimation		engg_flag,			@AllowMultipleRowSelection	engg_flag,		@MinimumValue		engg_seqno,			@MaximumValue			engg_seqno,	--TECH-73996
		@EditStepValue		engg_seqno,			@ClearTaskname			engg_name,			@ButtonNature		engg_name,			@InlineStyle			engg_name,	--TECH-75230
		@Showhistogram		engg_flag,			@CustomBorder			engg_flag,		--TECH-77097 --TECH-80135	
		--14469
		@Template			engg_name,			@TemplateId				engg_name,			@TplViewName		engg_name,			@TemplateTitle			engg_name,
		@Content			engg_nvarchar_max,	@CSSText				engg_nvarchar_max,	@Height				engg_seqno,			@Width					engg_seqno,
		@TemplateName		engg_name,			@TaskType			engg_name
		--14469
...
Begin
	...
	select	upper(control_id),	ExpandEvent,		ExpandAllEvent,		CollapseEvent,	
			...
			GanttControl,		ShowLines,			ShowHistogram	--TECH-66989 --TECH-77097
	from de_published_ui_control   ctrl (nolock),
	--es_comp_ctrl_type_mst_vw ctype (nolock)	left outer join 
	es_comp_ctrl_type_mst_extn  ext(nolock)	
	...
	BEGIN 
		...
		select @sql    = @sql+'ShowLines="'   + ltrim(rtrim(isnull(@ShowLines,'N')))    + '" '		--TECH-6698
		select @sql    = @sql+'ShowHistogram="'   + ltrim(rtrim(isnull(@ShowHistogram,'N')))    + '" '	--TECH-77097
	...
	KeyupSearch,					LiveClock,						ClearTask,					ShowAnimation,	--TECH-73996
	PreventMultipleRowSelection,	Minvalue,						Maxvalue,					Stepvalue,	--TECH-73996
	...
	@BorderBottomWidth,			@BorderTopWidth,			@BorderRightWidth,		@BorderLeftWidth,
	--TECH-69624
	@CtrlAssociateControl,		@CtrlButtonStyle,			@CtrlSelectOnlyLE,		
	@Searchable,				@LiveClock,					@ClearTask,				@ShowAnimation, --TECH-73996
	@AllowMultipleRowSelection,	@Minimumvalue,				@MaximumValue,			@EditStepValue,	--TECH-73996
	@ButtonNature,				@InlineStyle	--TECH-75230
	WHILE @@FETCH_STATUS = 0   
	BEGIN 
		...
		--FROM	de_ui_section_mobile_vw_fn  (@engg_customer_name,@engg_project_name,@engg_req_no)  mob, --TECH-79742
		...
		--TECH-73996
		SELECT @ClearTaskname	=	'' 

		IF ISNULL(@ClearTask,'N') = 'Y'
		BEGIN
		SELECT @ClearTaskname	=	upper(task_name)
			FROM de_published_action   act (NOLOCK),
			es_comp_ctrl_type_mst_extn  ext(NOLOCK)	
			WHERE act.customer_name  		= @engg_customer_name
			AND   act.project_name  		= @engg_project_name
			...
			AND  task_name	like '%ClUI'
			AND  task_descr like 'Clear Task for attachment of%'
		END
		--TECH-73996
	...
	BEGIN		
		...
		AND		rep.ecrno  	= act.ecrno --TECH-79742
 ...
 -- for systemgeneratedfielid TECH-52636 ends 
  ...
  --TECH-80135
	IF (ISNULL(@BorderStyle,'')<>''	OR  ISNULL(@BorderBottomRightRadius,'')<>''	OR  ISNULL(@BorderTopRightRadius,'')<>''	OR  ISNULL(@BorderBottomRightRadius,'')<>'' OR  ISNULL(@BorderTopLeftRadius,'')<>''
		OR  ISNULL(@BorderBottomColor,'')<>''	OR  ISNULL(@BorderTopColor,'')<>''	OR  ISNULL(@BorderRightColor,'')<>'' OR  ISNULL(@BorderLeftColor,'')<>''
		OR  ISNULL(@BorderBottomWidth,'')<>''	OR  ISNULL(@BorderTopWidth,'')<>''	OR  ISNULL(@BorderRightWidth,'')<>'' OR  ISNULL(@BorderLeftWidth,'')<>'')
	BEGIN       
	SELECT @CustomBorder ='Y' 
	END 
	--TECH-80135
	...
	----For stacked links configuration	PLF2.0_18487	Ends
		...
		--or ISNULL(@BorderLeftWidth,'') <> ''		--TECH-80135
		or ISNULL(@HasCustomAction,'') = 'Y'
		OR ISNULL(@CtrlAssociateControl,'') <> ''	OR ISNULL(@CtrlButtonStyle,'')			= 'Y'	OR ISNULL(@CtrlSelectOnlyLE,'')			= 'Y'--TECH-71262
		OR ISNULL(@Searchable,'')		= 'Y'		OR ISNULL(@LiveClock,'')				= 'Y'	OR ISNULL(@ClearTask,'')				= 'Y'	--TECH-73996
		OR ISNULL(@ShowAnimation,'')	= 'Y'		OR ISNULL(@AllowMultipleRowSelection,'')IN ('Y','N')	OR ISNULL(@MinimumValue,0) > 0 --TECH-73996
		OR ISNULL(@MaximumValue,0) > 0				OR ISNULL(@EditStepValue,0) > 0		--TECH-73996
		OR ISNULL(@PreTask,'') <> ''				OR ISNULL(@PostTask,'') <> ''					OR ISNULL(@BrowsePreTask,'') <> ''				OR ISNULL(@BrowsePostTask,'') <> '' --TECH-75230
		OR ISNULL(@DeletePreTask,'') <> ''			OR ISNULL(@DeletePostTask,'') <> ''				OR ISNULL(@ButtonNature,'')	<> ''				OR ISNULL(@InlineStyle,'')	  <> ''	--TECH-75230
		--TECH-80135
		OR ISNULL(@BorderStyle,'')<>''			OR  ISNULL(@BorderBottomRightRadius,'')<>''	OR  ISNULL(@BorderTopRightRadius,'')<>''	OR  ISNULL(@BorderBottomRightRadius,'')<>'' OR  ISNULL(@BorderTopLeftRadius,'')<>''
		OR  ISNULL(@BorderBottomColor,'')<>''	OR  ISNULL(@BorderTopColor,'')<>''	OR  ISNULL(@BorderRightColor,'')<>'' OR  ISNULL(@BorderLeftColor,'')<>''
		OR  ISNULL(@BorderBottomWidth,'')<>''	OR  ISNULL(@BorderTopWidth,'')<>''	OR  ISNULL(@BorderRightWidth,'')<>'' OR  ISNULL(@BorderLeftWidth,'')<>''
		--TECH-80135
        ...
        IF LEN(@placeholder) > 0 --Code modified for TECH-74150
...
-- TECH-45171 starts
  ...
  --Code commented for TECH-72927 on 20sep2022 starts
		--if (isnull(@autoheight,'') =  'y')
		--	select @sql    = @sql+'AutoHeight="'			+ ltrim(rtrim(isnull(upper(@autoheight),'')))	+ '" '  	
  --Code commented for TECH-72927 on 20sep2022 ends
...
--- starts TECH-52636
   ...
   --ends TECH-52636
		...
		--TECH-73996
		IF (ISNULL(@Searchable,'') = 'y')
		BEGIN
			SELECT @sql    = @sql+'Searchable="'			+ LTRIM(RTRIM(ISNULL(@Searchable,'')))	+ '" '  
		END

		IF (ISNULL(@LiveClock,'') = 'y')
		BEGIN
			SELECT @sql    = @sql+'LiveClock="'			+ LTRIM(RTRIM(ISNULL(@LiveClock,'')))	+ '" '  
		END

		IF (ISNULL(@ClearTask,'') = 'y')
		BEGIN
			SELECT @sql    = @sql+'ClearTask="'			+ LTRIM(RTRIM(ISNULL(@ClearTaskName,'')))	+ '" '  
		END

		IF (ISNULL(@ShowAnimation,'') = 'y')
		BEGIN
			SELECT @sql    = @sql+'ShowAnimation="'			+ LTRIM(RTRIM(ISNULL(@ShowAnimation,'')))	+ '" '  
		END

		IF (ISNULL(@AllowMultipleRowSelection,'') IN ('Y','N'))
		BEGIN
			IF ISNULL(@AllowMultipleRowSelection,'') = 'Y'
			BEGIN
				SELECT @AllowMultipleRowSelection = 'N'
			END
			ELSE IF ISNULL(@AllowMultipleRowSelection,'') = 'N'
			BEGIN
				SELECT @AllowMultipleRowSelection = 'N'
			END

			SELECT @sql    = @sql+'AllowMultipleRowSelection="'			+ LTRIM(RTRIM(ISNULL(@AllowMultipleRowSelection,'')))	+ '" '  
		END

		IF (ISNULL(@MinimumValue,0) > 0)
		BEGIN
			SELECT @sql    = @sql+'MinValue="'			+ LTRIM(RTRIM(ISNULL(@MinimumValue,0)))	+ '" '  
		END

		IF (ISNULL(@MaximumValue,0) > 0)
		BEGIN
			SELECT @sql    = @sql+'MaxValue="'			+ LTRIM(RTRIM(ISNULL(@MaximumValue,0)))	+ '" '  
		END

		IF (ISNULL(@EditStepValue,0) > 0)
		BEGIN
			SELECT @sql    = @sql+'StepValue="'			+ LTRIM(RTRIM(ISNULL(@EditStepValue,0)))	+ '" '  
		END
		--TECH-73996

		--TECH-75230
		IF (ISNULL(@ButtonNature,'') <> '')
		BEGIN
			SELECT @sql    = @sql+'ButtonNature="'			+ LTRIM(RTRIM(ISNULL(@ButtonNature,'')))	+ '" '  
		END

		IF (ISNULL(@InlineStyle,'') <> '')
		BEGIN
			SELECT @sql    = @sql+'InlineStyle="'			+ LTRIM(RTRIM(ISNULL(@InlineStyle,'')))	+ '" '  
		END
		--TECH-75230
...
-- Code Added for the Defect ID TECH-27286 Ends
		...
		IF ISNULL(@CustomBorder,'')		= 'Y' --TECH-80135
		BEGIN
	...
	FROM	de_ui_section_mobile_vw_fn  (@engg_customer_name,@engg_project_name,@engg_req_no)  sec --TECH-79742
 ...
 --   and		sec.mobilegridas	= 'ControlType'
	...
	--TECH-72114
			Select	@IsList			= ISNULL(IsList,'N'),
					@MultiSelect	= ISNULL(MultiSelect,'N'),
					@ListItemType	= ISNULL(ListItemType,'') --TECH-73996
			from	es_comp_ctrl_type_mst_extn(nolock)	
			where	customer_name	=  @engg_customer_name
			and		project_name	= @engg_project_name
			and		process_name	= @process_name
  ...
  --     and           sec.section_type		= 'MobileGrid'   
	...
	SELECT @control_type = ''
		...
		FROM	--de_ui_section_mobile sec (NOLOCK),
				de_ui_section_mobile_vw_fn  (@engg_customer_name,@engg_project_name,@engg_req_no)  sec, --TECH-79742
				de_published_ui_control ctl (nolock)
		WHERE	sec.customer_name	= @engg_customer_name
		AND		sec.project_name	= @engg_project_name
		AND		sec.ecrno			= @engg_req_no
...
---- For MobGrid Control Ends
	...
	AssociateControl,	ButtonStyle,			SelectOnlyListValues,  --TECH-71262
	KeyupSearch,		Minvalue,	--TECH-73996
	MaxValue,			StepValue,	--TECH-73996
	ctrl.section_bt_synonym			--TECH-76231
	from de_published_ui_grid   ctrl (nolock),
	es_comp_ctrl_type_mst_vw ctype (nolock) left outer join 
	es_comp_ctrl_type_mst_extn  ext (nolock)
	on		ctype.customer_name		= ext.customer_name
	...
	and  ctrl.activity_name  	= @activity_name
	and  ctrl.ui_name    		= @ui_name_tmp
	ORDER BY ctrl.column_no	--TECH-76231
	...
	@Searchable,		@MinimumValue,				@MaximumValue,		@EditStepValue,	--TECH-73996
	@section_bt_synonym	--TECH-76231
	...
	BEGIN		
		...
		SELECT	@ReportName			= rep.report_name, 
				@ReportContext		= rep.report_context
		FROM	es_comp_ctrl_type_mst mst (nolock),
				De_published_report_attributes rep (nolock), --TECH-74072
				de_published_action act (nolock) --TECH-74072
		WHERE	rep.customer_name  	= @engg_customer_name
		AND		rep.project_name  	= @engg_project_name
		AND		rep.ecrno    		= @engg_req_no
		AND		rep.process_name  	= @process_name
		...
		SELECT	@SPReportName		= rep.report_name, 
				@SPReportContext	= rep.report_context
		FROM	es_comp_ctrl_type_mst mst (nolock),
				de_published_sp_report_action_segment rep (nolock), --TECH-74072
				de_published_action act (nolock) --TECH-74072
		WHERE	rep.customer_name  	= @engg_customer_name
		AND		rep.project_name  	= @engg_project_name
		AND		rep.ecrno    		= @engg_req_no
 ...
 select @autofilename ='n'      
	...
	-- To get the Report Task enabled columns for Mobility --TECH-27286 Ends
		...
		IF EXISTS	(
			SELECT	'X'
			FROM	ep_tpl_template_param_mapping	(NOLOCK)
		...
		or isnull(@QlikLink,'N')			= 'y'	or isnull(@SystemGeneratedFileId,'')	=  'y'	or isnull(@Ishijri,'')					= 'y'	or ISNULL(@IsMarquee,'N')			= 'Y'
		or ISNULL(@configuration,'')		<>''	or ISNULL(@IsAssorted,'N')				= 'y'--PLF2.0_18487	
		or isnull(@RatingType,'')			 <>''	or ISNULL(@CaptchaData,'')				<> ''	or Len(@ReportName)						>= 1	or LEN(@SPReportName)				>= 1 --TECH-74072
		or isnull(@gridtoform,'N')			= 'y'	or isnull(@rowexpander,'N')				= 'y'	--or isnull(@multiselector,'N')			= 'y'
		...
		OR ISNULL(@Searchable,'N')			= 'y'	 --TECH-73996
		OR ISNULL(@PreTask,'') <> ''				OR ISNULL(@PostTask,'') <> ''					OR ISNULL(@BrowsePreTask,'') <> ''				OR ISNULL(@BrowsePostTask,'') <> '' --TECH-75230
		OR ISNULL(@DeletePreTask,'') <> ''			OR ISNULL(@DeletePostTask,'') <> ''	
		OR ISNULL(@columnclass,'') <> ''			OR ISNULL(@Template,'N') = 'y' -- TECH-76231	--14469
	...
	--SELECT	@columnclass	= columnclass
	...
	--AND		column_bt_synonym	= @controlcolumnbt
	--AND		charindex('~', columnclass) > 0 
	----TECH-76231

	--TECH-76402
	Declare	colcur_swipe  cursor FAST_FORWARD for
	...
	open colcur_swipe 
	fetch next from colcur_swipe into @columnname, @control_bt_synonym, @view_name, @columnclass
	WHILE @@FETCH_STATUS = 0   
	BEGIN 
	
	IF @columnname = @controlcolumnbt
	BEGIN
	...
	End
	Close colcur_swipe
	deallocate colcur_swipe
	----TECH-76402
	-------List Swipe Action Ends	
	--TECH-76231
	-- commented by jeya starts
	...
	--	end
		...
		FROM	ep_tpl_template_param_mapping	a (NOLOCK)
		JOIN	ep_tpl_template b (NOLOCK)
		...
		AND		a.ActivityName	=	@activity_name
		AND		a.UIName		=	@ui_name_tmp
		AND		a.BTsynonym		=	@controlcolumnbt
		
		SELECT @xml_seq_tmp_out=@xml_seq_tmp_out+1

		SELECT @sql = '<Template Name="'   + LTRIM(RTRIM(ISNULL(@templatename,'')))   + '" ' +
					  'ModeFlag="S" ' +
					  'ViewName="'  + LTRIM(RTRIM(ISNULL(@tplviewname,'')))    + '" ' +
					  'Sequence="" ' +
					  'Group="" ' +
					  'Title="'   + LTRIM(RTRIM(ISNULL(@TemplateTitle,'')))   + '" ' +
					  'Icon="" ' +
					  'Content="'   + LTRIM(RTRIM(ISNULL(@content,'')))   + '" ' +
					  'CSS="" ' +
					  'CSSText="'   + LTRIM(RTRIM(ISNULL(@cssText,'')))   + '" ' +
					  'Height=" '   + LTRIM(RTRIM(ISNULL(@height,0)))   + '" ' +
					  'Width="'   + LTRIM(RTRIM(ISNULL(@width,0)))   + '" ' +
					  'Sizeable="" ' 
	...
	IF EXISTS ( SELECT	'X'
	FROM	ep_tpl_template_action_mapping  (nolock)
	WHERE	customername		= @engg_customer_name
	AND		projectname			= @engg_project_name
	AND		processname			= @process_name
	AND		componentname		= @engg_component
	...
	select @xml_seq_tmp_out=@xml_seq_tmp_out+1
			...
			values
			(@engg_customer_name, @engg_project_name, @engg_req_no,   @process_name, @engg_component,
			@activity_name,  @guid,    '<Actions>',@xml_seq_tmp_out)
	

			
		DECLARE @ActionName	engg_name, @ActionType	engg_name, @taskname	engg_name
		Declare	ActionTemplate  cursor FAST_FORWARD for
		select	Distinct ActionName,	ActionType, TaskName
		from	ep_tpl_template_action_mapping  (nolock)
		where	customername		= @engg_customer_name
		and		projectname			= @engg_project_name
		and		processname			= @process_name
		...
		open ActionTemplate 
		fetch next from ActionTemplate into @ActionName, @ActionType, @taskname
		WHILE @@FETCH_STATUS = 0   
		BEGIN 

			IF @ActionType = 'TaskInfo'
				SELECT @TaskType = 'Trans'
			ELSE IF @ActionType = 'LinkInfo'
				SELECT @TaskType = 'Link'

			
			select @xml_seq_tmp_out=@xml_seq_tmp_out+1
			select @sql =	'<Action Name="'  + LTRIM(RTRIM(ISNULL(@ActionName,'')))    + '" ' +
							'sequence="" ' +
						  'modeFlag="S" ' +
						  
						  'actionType="'  + LTRIM(RTRIM(ISNULL(@ActionType,'')))    + '" ' +
						  'actionParams="" ' +
						  'taskName="'   + LTRIM(RTRIM(ISNULL(@taskname,'')))   + '" ' +
						  'taskType=" ' + LTRIM(RTRIM(ISNULL(@TaskType,'')))   + '" ' +
						  'reportTask="No" ' 
			SELECT @sql = @sql +'/>'
	...
	BEGIN
			...
			values
			(@engg_customer_name, @engg_project_name, @engg_req_no,   @process_name, @engg_component,
			@activity_name,  @guid,    '<Params>',@xml_seq_tmp_out)

		DECLARE @ParamName	engg_name, @param_mappedviewname	engg_name, @param_datatype	engg_name, @param_sampledata	engg_name
		Declare	ParamTemplate  cursor FAST_FORWARD for
		select	 ParameterName,	MappedViewName,	'', SampleData
		from	ep_tpl_template_param_mapping  (nolock)
		where	customername		= @engg_customer_name
		and		projectname			= @engg_project_name
		and		processname			= @process_name
		...
		open ParamTemplate 
		fetch next from ParamTemplate into @ParamName, @param_mappedviewname, @param_datatype, @param_sampledata
		WHILE @@FETCH_STATUS = 0   
		BEGIN 
			...
			select @xml_seq_tmp_out=@xml_seq_tmp_out+1
			select @sql =	'<Param Name="' + LTRIM(RTRIM(ISNULL(@ParamName,'')))   + '" ' +
							'sequence="" ' +
							
						  'ModeFlag="S" ' +
						  'MappedViewName="'  + LTRIM(RTRIM(ISNULL(@param_mappedviewname,'')))    + '" ' +
						  'DataType="'  + LTRIM(RTRIM(ISNULL(@param_datatype,'')))    + '" ' +
						  'SampleData="'   + LTRIM(RTRIM(ISNULL(@param_sampledata,'')))   + '" ' +
						  '/>'
	...
	END
		...
		values
		(@engg_customer_name, @engg_project_name, @engg_req_no,   @process_name, @engg_component,
		@activity_name,  @guid,    '</Template>',@xml_seq_tmp_out)
	END	
		select @xml_seq_tmp_out=@xml_seq_tmp_out+1
		insert into ep_genxml_tmp
		(customer_name,   project_name,   req_no,   process_name,  component_name,
	...
	-------Callout in Grid for mobility Starts
	--------------List Swipe Action Starts
	--Code commented for TECH-76231 starts
	--Declare	colcur  cursor FAST_FORWARD for
	--select	Distinct upper(column_bt_synonym), upper(control_id), view_name, columnclass
	--from	de_published_ui_grid grd (nolock)
	--where	customer_name	= @engg_customer_name
	...
	--deallocate colcur
	---------List Swipe Action Ends
	--Code commented for TECH-76231 ends
...
IF EXISTS( SELECT 'Y' FROM sysobjects WHERE name = 'de_ui_extended_properties' AND TYPE = 'P')
BEGIN
	GRANT EXEC ON de_ui_extended_properties TO PUBLIC
END
GO
